...
<main class="ml-0 sm:ml-[80px] max-w-[98%] mx-auto px-2 py-6">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-yellow-200 p-8 max-w-4xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        {{-- [BARU] Tambah ID --}}
                        <input type="text" id="input_nomor_spm" name="nomor_spm" 
                    ...
                    <div>
                        ...
                        {{-- [BARU] Tambah ID --}}
                        <input type="date" id="input_tanggal_spm" name="tanggal_spm" 
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        {{-- [BARU] Tambah ID --}}
                        <input type="text" id="input_nomor_sp2d" name="nomor_sp2d" 
                    ...
                    <div>
                        ...
                        {{-- [BARU] Tambah ID --}}
                        <input type="date" id="input_tanggal_sp2d" name="tanggal_sp2d" 
        ...
        <div class="flex justify-between pt-6 border-t gap-4">
            ...
            {{-- [BARU] Tambahkan disabled & class --}}
            <button 
                type="button" 
                id="openApproveModal"
                disabled
                class="bg-green-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-green-700 shadow-lg flex items-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        // [BARU] LOGIKA CEK INPUT
        const inputNomorSPM = document.getElementById('input_nomor_spm');
        const inputTglSPM = document.getElementById('input_tanggal_spm');
        const inputNomorSP2D = document.getElementById('input_nomor_sp2d');
        const inputTglSP2D = document.getElementById('input_tanggal_sp2d');
        const btnApprove = document.getElementById('openApproveModal');

        function checkInputs() {
            if(inputNomorSPM && inputTglSPM && inputNomorSP2D && inputTglSP2D && btnApprove) {
                // Cek isi
                const isComplete = 
                    inputNomorSPM.value.trim() !== "" && 
                    inputTglSPM.value.trim() !== "" &&
                    inputNomorSP2D.value.trim() !== "" && 
                    inputTglSP2D.value.trim() !== "";

                // Disable/Enable
                btnApprove.disabled = !isComplete;
                
                // Ubah style
                if (isComplete) {
                    btnApprove.classList.remove('bg-gray-400', 'cursor-not-allowed');
                    btnApprove.classList.add('bg-green-600', 'hover:bg-green-700');
                } else {
                    btnApprove.classList.add('bg-gray-400', 'cursor-not-allowed');
        ...
        if(inputNomorSPM) {
            const inputs = [inputNomorSPM, inputTglSPM, inputNomorSP2D, inputTglSP2D];
            inputs.forEach(inp => {
                inp.addEventListener('input', checkInputs);
                inp.addEventListener('change', checkInputs);
            });
            checkInputs(); // Run once
        }

        // --- KODE LAMA BAWAH (MODAL DLL) ---
