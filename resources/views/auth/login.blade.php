...
<body class="relative min-h-screen bg-login-pattern bg-cover bg-center bg-no-repeat">
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center px-4 py-6 sm:px-0">
        <div class="bg-white rounded-2xl border-2 border-blue-500 shadow-lg
                    w-full max-w-[320px] sm:max-w-sm px-4 py-5 sm:px-8 sm:py-8">

            <div class="flex flex-col items-center">
                <img src="{{ asset('img/logo_kementerian_desa.png') }}" alt="Logo"
                    class="w-20 h-20 sm:w-24 sm:h-24 mt-1 mb-4 sm:mb-5 drop-shadow" />
                <p class="text-center text-lg sm:text-xl text-[#2954B0] font-bold">Inspektorat Jenderal</p>
                <p class="text-center text-xs sm:text-sm text-gray-700 font-medium mb-4 sm:mb-5">
            ...
            <form method="POST" action="{{ route('login.attempt') }}" class="w-full">
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        ...
                        <input id="nip" name="nip" type="text" value="{{ old('nip') }}" required autofocus
                            autocomplete="username" aria-invalid="{{ $errors->has('nip') ? 'true' : 'false' }}" class="w-full text-sm pl-11 pr-4 py-2 rounded-full border 
                               {{ $errors->has('nip') ? 'border-red-400' : 'border-blue-200' }}
                               focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Masukkan NIP/NIK">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        ...
                        <input id="password" name="password" type="password" required autocomplete="current-password"
                            aria-label="Password" aria-invalid="{{ $errors->has('password') ? 'true' : 'false' }}"
                            class="w-full text-sm pl-11 pr-11 py-2 rounded-full border
                               {{ $errors->has('password') ? 'border-red-400' : 'border-blue-200' }}
                               focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Masukkan password">
                ...
                <div class="flex items-center justify-between mt-4">
                    <div class="flex items-center">
                        <input id="remember_me" type="checkbox" name="remember" class="h-3 w-3 rounded border-gray-300">
