...
<main class="transition-all duration-300 ml-0 sm:ml-[60px]">
    ...
    {{-- BACK LINK & TITLE (Style detail.php) --}}
    ...
    @php
        ...
        // Logika Data & Status (Gabungan Lengkap)
        ...
        // Priority Logic untuk pewarnaan status
        if ($lowerStatus === 'diselesaikan manual' || str_contains($lowerStatus, 'diselesaikan manual')) {
            $statusClass = 'bg-amber-100 text-amber-800 border-amber-200';
        } elseif (str_contains($lowerStatus, 'berlangsung')) {
            $statusClass = 'bg-yellow-100 text-yellow-700 border-yellow-200';
        } elseif (str_contains($lowerStatus, 'selesai')) {
            $statusClass = 'bg-green-100 text-green-700 border-green-200';
        } elseif (str_contains($lowerStatus, 'pembuatan')) {
            $statusClass = 'bg-blue-100 text-blue-700 border-blue-200';
        } elseif (str_contains($lowerStatus, 'tindakan')) {
        ...
        if (str_contains(strtolower($statusLap), 'disetujui') || str_contains(strtolower($statusLap), 'valid')) {
            $statusLapClass = 'bg-green-100 text-green-700 border-green-200';
        } elseif (str_contains(strtolower($statusLap), 'revisi') || str_contains(strtolower($statusLap), 'perlu')) {
    ...
    {{-- ================= HEADER PERJADIN (Style detail.php) ================= --}}
    ...
    {{-- ALERT: Diselesaikan Manual (Fitur detail.php) --}}
    @php
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6">
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    @php $alasanManual = trim($perjadin->alasan_selesai_manual ?? ''); @endphp
                    ...
                    <p class="text-xs text-amber-700 mt-2">
                        Terdapat <strong>{{ $pegawaiBelum }}</strong> pegawai yang tercatat
            ...
            @else
                <div class="space-y-3">
                    @foreach($pegawai as $pg)
                        @php $finished = (int) $pg->is_finished === 1; @endphp
                        <div class="flex items-start justify-between rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                            <div class="space-y-0.5">
                                ...
                                <p class="text-xs text-gray-500">NIP: {{ $pg->nip }}</p>
                                @if(!empty($pg->no_telp))
                                    <p class="text-xs text-gray-500">
                                        <i class="fa-solid fa-phone mr-1 text-gray-400"></i> {{ $pg->no_telp }}
                                    </p>
                                @endif
                                @if(!empty($pg->email))
                                    <p class="text-xs text-gray-500">
                                        <i class="fa-solid fa-envelope mr-1 text-gray-400"></i> {{ $pg->email }}
                                    </p>
                                @endif
                            </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                            <div class="h-2 rounded-full bg-emerald-500" style="width: {{ $progress['persen_pegawai_selesai'] }}%;"></div>
                        </div>
                    </div>
                </div>
    ...
    {{-- ================= URAIAN PELAKSANAAN (MERGED FEATURE) ================= --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        <h2 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
        ...
        {{-- 1. Uraian PIC / Surat Tugas (Fitur dari detail.php) --}}
        <div class="mb-6">
        ...
        {{-- 2. Uraian Individu Pegawai (Fitur Tambahan dari detail.blade.php) --}}
        <div class="border-t border-gray-100 pt-6">
            <div class="flex items-center justify-between mb-3">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                    Uraian Individu Pegawai ({{ isset($uraianIndividu) ? $uraianIndividu->count() : 0 }})
                </p>
            </div>
            
            @if(isset($uraianIndividu) && $uraianIndividu->isNotEmpty())
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-10">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <select id="geotagFilterDate" class="border border-gray-300 rounded-lg text-xs px-2 py-1"></select>
                </div>
            </div>

            <div id="mapPimpinan" class="rounded-xl border border-gray-200 w-full h-80"></div>
    ...
    @if($geotagSummary['total_record'] > 0)
        ...
        <style>
            /* Fix z-index Leaflet agar tidak menimpa navbar */
