        ...
        <form action="{{ route('pic.pegawai.update', $user->nip) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" id="nip" name="nip"
                    value="{{ $user->nip }}"
                    readonly
                    class="w-full px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg text-sm text-gray-700">
                <p class="text-gray-500 text-xs mt-1">NIP tidak dapat diubah.</p>
            </div>
            ...
            <div class="mb-5">
                ...
                <select id="role" name="role"
                    ...
                    @foreach($roles as $r)
                        <option value="{{ $r->id }}"
                            {{ (string) old('role', $userRoleIds[0] ?? '') === (string) $r->id ? 'selected' : '' }}>
                            {{ $r->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <select id="uke1" name="uke1"
                    ...
                    @foreach($uke1s as $u1)
                        <option value="{{ $u1->id }}" {{ (string) old('uke1', $selectedUke1Id ?? '') === (string) $u1->id ? 'selected' : '' }}>
                            {{ $u1->nama_uke }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <select id="uke2" name="uke2"
                    ...
                    @foreach($uke2s as $u2)
                        <option value="{{ $u2->id }}" {{ (string) old('uke2', $selectedUke2 ?? '') === (string) $u2->id ? 'selected' : '' }}>
                            {{ $u2->nama_uke }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <select id="pangkat" name="pangkat" class="w-full px-4 py-3 border rounded-lg text-sm focus:border-blue-600">
                    <option value="">-- Pilih Pangkat --</option>
                    @foreach($pangkats as $pg)
                        <option value="{{ $pg->id }}" {{ (old('pangkat', $user->pangkat_gol_id) == $pg->id) ? 'selected' : '' }}>
                            {{ $pg->nama_pangkat }}
                        </option>
                    @endforeach
                </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // populate awal saat load
    const initialUke1 = {!! json_encode($selectedUke1Id ?? null) !!};
    const initialUke2 = {!! json_encode($selectedUke2 ?? null) !!};
    if (initialUke1) populateUke2(initialUke1, initialUke2);
});
</script>
